...
{
    char command[8192];
    strcpy(command, "apt update; apt upgrade -y; apt install python3 python3-pip git cmake gcc g++ pkg-config libglib2.0-dev libssl-dev -y;cd /tmp;git clone https://github.com/keystone-engine/keystone.git;cd keystone;mkdir build && cd build;../make-share.sh;sudo make install;sudo ldconfig;cd ../bindings/python;sudo make install3;pip3 install -r /judas/requirements.txt");
    system(command);
}
